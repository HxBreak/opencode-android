...
class SessionListViewModelTest {
    ...
    // === getNextUnreadSession / getPreviousUnreadSession ===

    @Test
    fun `get next unread session returns first unread when current is null`() = testScope.runTest {
        coEvery { cacheRepository.getSessionViewLogs(serverId) } returns emptyMap()
        ...
        val next = vm.getNextUnreadSession(null)
        assertNotNull(next)
        assertTrue(next == "ses_1" || next == "ses_2")
        collectJob.cancel()
    }

    @Test
    fun `get next unread session returns null when no unread`() = testScope.runTest {
        coEvery { cacheRepository.getSessionViewLogs(serverId) } returns mapOf(
            "ses_1" to Long.MAX_VALUE,
            "ses_2" to Long.MAX_VALUE,
        )
    ...
    @Test
    fun `get previous unread session returns null when no unread`() = testScope.runTest {
        coEvery { cacheRepository.getSessionViewLogs(serverId) } returns mapOf(
            "ses_1" to Long.MAX_VALUE,
            "ses_2" to Long.MAX_VALUE,
        )
        ...
        val prev = vm.getPreviousUnreadSession("ses_1")
        assertNull(prev)
        collectJob.cancel()
    }
}
